...
def vision_node(state: "AgentState"):    
    ...
    - item_name: 商品名を推定。簡潔に記載し、括弧書きや補足説明は含めない
      - ブランド名がわかる場合は必ず含める
      - 型番がわかる場合は含める
      - 例: "Louis Vuitton ネヴァーフル MM"、"SEIKO プレザージュ SARX035"、"NIKE Free RN Flyknit"
    - visual_features: 視覚的特徴をリストで抽出
      ...
      - 状態（新品同様、使用感あり等）
      - その他の特徴的な要素
    visua_featuresで色を示す際はシンプルな単語で、最も広い面積を占めている色を出力してください（例："赤"、"青"）
    フリマサイトなどでは色を記載する際にシンプルにすることが多いからです
    """
