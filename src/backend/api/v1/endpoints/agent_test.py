...
@router.post("/agent/price_test")
async def test_price_agent(request: AgentVisionRequest):
    """
    価格検索エージェントのテスト用エンドポイント

    vision_node → search_node → price_node の一連フローをテスト
    - processable かつ mass_productの場合: 価格検索まで実行
    - それ以外の場合: price_outputはNone
    """
    try:
        image_content = request.image_data
